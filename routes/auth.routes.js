...
// GET /auth/users - Récupère tous les utilisateurs
router.get("/users", isAuthenticated, (req, res, next) => {
    User.find()
        .then((users) => {
            res.status(200).json(users);  // Retourne tous les utilisateurs
        })
        .catch((err) => next(err));  // Gérer les erreurs
});

// GET /auth/users/:id - Récupère un utilisateur par son ID
